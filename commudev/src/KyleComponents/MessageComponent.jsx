...
import { Send, Smile, Paperclip, Image, Mic, Phone, VideoIcon } from 'lucide-react';
    ...
    <div className="messagePage">
      ...
      <div className="messagePage__leftSidebar">
        ...
        <div className="messagePage__usersList">
          {users.map((user) => (
            <div 
              key={user.id}
              className={`messagePage__userItem ${selectedUser.id === user.id ? 'messagePage__userItem--active' : ''}`}
      ...
      <div className="messagePage__main">
        ...
        <div className="messagePage__navbar">
          <div className="messagePage__navIcons">
            {navigationItems.map((item, index) => (
              <Link
                key={index}
                to={item.path || `/${item.label.toLowerCase().replace(" ", "")}`}
                className="messagePage__navItem"
              >
                <img 
                  src={item.icon} 
                  alt={`${item.label} icon`} 
                  className="messagePage__navIcon" 
                />
              </Link>
            ))}
          </div>
            ...
            {messages.map((msg) => (
              <div 
                ...
                className={`messagePage__message ${
                  msg.sender === 'me' ? 'messagePage__message--sent' : 'messagePage__message--received'
                }`}
              >
        ...
        <div className="messagePage__userDetails">
          ...
          <section className="messagePage__detailsSection">
            ...
            <div className="messagePage__mediaGrid">
              {[1, 2, 3, 4].map((num) => (
                <div key={num} className="messagePage__mediaItem">
                  <img src={`/api/placeholder/80/80`} alt={`Media ${num}`} />
                </div>
              ))}
            </div>
          </section>
